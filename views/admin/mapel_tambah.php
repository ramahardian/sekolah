...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($error === '') {
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO mapel (kode_mapel, nama_mapel, guru_id) VALUES (?, ?, ?)");
            $stmt->execute([$kode_mapel, $nama_mapel, $primaryGuruId]);
            $mapelId = (int) $pdo->lastInsertId();

            if ($selectedGuruIds) {
                $insert = $pdo->prepare("INSERT INTO mapel_guru (mapel_id, guru_id) VALUES (?, ?)");
                foreach ($selectedGuruIds as $gid) {
                    $insert->execute([$mapelId, $gid]);
                }
            }

            set_flash_message('success', 'Mapel berhasil ditambahkan.');
            echo "<script>window.location.href='index.php?page=mapel';</script>";
            exit;
        } catch (PDOException $e) {
            $error = "Gagal menambah mapel: " . $e->getMessage();
        }
    }
}
?>
