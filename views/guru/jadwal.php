...
$query_sql = "SELECT j.*, k.nama_kelas, m.nama_mapel
              FROM jadwal_pelajaran j
              JOIN kelas k ON j.kelas_id = k.id
              JOIN mapel m ON j.mapel_id = m.id
              WHERE j.guru_id = :guru_id";

$params = ['guru_id' => $guru['id']];
...
<div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
    <div>
        ...
        <p class="text-gray-500 text-sm">Jadwal mengajar untuk <?= htmlspecialchars($guru['nama_guru'], ENT_QUOTES, 'UTF-8') ?>.</p>
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="p-4 border-b border-gray-100 bg-gray-50/50">
        <form action="index.php" method="GET" class="flex flex-col md:flex-row gap-3">
            <input type="hidden" name="page" value="guru-jadwal">
            <select name="hari"
